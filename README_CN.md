...
- ⬆️ **一键在线自动更新**——管理员在界面里升级正在运行的服务栈（拉取 → 迁移 → 重建 →
  健康闸门 → 失败自动回滚）。
- 🌐 **任意模型**——Anthropic 兼容网关（默认 ARK / 火山引擎）。GLM、DeepSeek、doubao、GPT、
  Qwen……取决于你的账号或网关暴露了什么；菜单只列探测健康的模型。
...
MIT——见 [LICENSE](LICENSE)。Kin 构建于
[Claude Agent SDK](https://www.npmjs.com/package/@anthropic-ai/claude-agent-sdk) 之上，受
[Anthropic 商业条款](https://www.anthropic.com/legal/commercial-terms)约束。完整的第三方
归属见 [NOTICE](NOTICE)。
