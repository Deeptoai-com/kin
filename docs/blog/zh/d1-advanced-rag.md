...
> 🏁 **本篇是设计篇里唯一一篇"已整条落地上生产"的**（2026-06-13，`rag-line` → PR #153–#182，`RAG_ENABLED` flag 控制）。标题那趟"从空架子到 Agent 自己会调的检索工具"的路——**走完了**。所以这不再是一篇"该有却没有"的设计稿，而是一条完整的**「设计 → 现实」弧线**：前半是 2026-06-07 当初的推理，后半（[实现回填](#实现回填2026-06-13设计落地了但有五处打脸)）是六天后真做出来时、设计被现实纠正的五处。

> 这一篇开启系列的**设计篇**——按 references（尤其 baby-agent 第七章 Agentic RAG）倒推 oxygenie 当时**该有、却还没有**的能力。RAG 正是最典型的缺口：2026-06-07 时 `documentChunks` 表早带了 `embedding` 向量列，却**一行都没写进去过**。本文回答两件事：① 检索现状当时空在哪（有 `文件:行号` 为证）；② 在 per-message worker + MCP 架构里 Advanced RAG **该怎么落**——答案颠覆一个直觉：**RAG 不是喂给 prompt 的前处理管线，是 Agent 自己会调的一个工具。** 而这个判断，下文会用生产代码验证：**对得漂亮，也被打脸五处。**
...
- **检索范围由用户在对话里勾选。** 设计假设 `kbId` 由会话配置决定；落地做成了**对话里临时勾选知识库**（onyx 轻量式）：面板勾选 → `selectedKbIds` 穿过 chat 消息 → worker → `kb_search` 的 `kbIds[]`（`search.ts` 多 KB 取文件并集）。**检索范围是用户每轮可调的状态，不是绑死的配置**——这更贴合"非技术用户按需圈定资料"的真实用法。
...
🏁 状态：**已整条落地上生产**（设计篇里唯一一篇）。"空架子"部分是 2026-06-07 起点快照；设计部分已于 **2026-06-13 全量上线**（`rag-line` → PR #153–#182，`RAG_ENABLED` flag 控制），真实代码与"五处打脸 + 最后一公里"见 [实现回填](#实现回填2026-06-13设计落地了但有五处打脸)。
