# Dokploy Deployment (Kin) — LEGACY / not the current path

> ⚠️ **Legacy — not currently used.** Production runs on the **VPS** path
> ([one-command `install-vps.sh`](../../scripts/install-vps.sh) / [docker-compose.md](docker-compose.md))
> and the **Cloudflare Tunnel** path ([tunnel.md](tunnel.md)) — **not** Dokploy. This guide is
> kept for reference only; some details (image refs, off-server build) predate the prebuilt
> **multi-arch GHCR images** that today's paths simply *pull*. Prefer the VPS or tunnel guides.

> **Historical status: verified working** — `kin.example.com` was deployed to Dokploy on
> 2026-06-05. This guide reflects that *actual, working* procedure plus every blocker hit and
> its root cause. Read the **Critical invariants** box before you start — most deployment
> failures came from getting one of those wrong.

Kin runs as a **Docker Compose** stack on Dokploy (Traefik in front). The app
...
5. **Previews use a SINGLE-level wildcard `*.<domain>`** (e.g. `*.kin.example.com`).
...
- A domain on Cloudflare (this guide uses `kin.example.com`).
...
| Name | Type | Content |
|---|---|---|
| `kin.example.com` | A | `<dokploy-host-ip>` |
| `*.kin.example.com` | A | `<dokploy-host-ip>` |

> Use a **single-level** wildcard `*.kin.example.com`. CF free SSL covers it; it does NOT
> cover `*.preview.kin.example.com`. App lives at the apex; previews are random
> `<id>.kin.example.com` subdomains.

## Step 2 — Cloudflare TLS (Full Strict + Origin CA)

1. SSL/TLS → Overview → set encryption mode to **Full (Strict)**.
2. SSL/TLS → Origin Server → **Create Certificate** → hostnames `kin.example.com` and
   `*.kin.example.com`, validity 15 years → save the **cert** + **private key**.
...
docker buildx create --name kinbuilder --driver docker-container --bootstrap --use

# build amd64 + push (Dokploy hosts are x86_64)
docker buildx build --builder kinbuilder --platform linux/amd64 \
...
- Make it **public**: GitHub → your packages → `kin/app` → Package settings →
...
# --- identity / domain ---
APP_NAME=kin
APP_NAME_SANITIZED=kin-cc      # ⚠️ MUST be unique across all stacks on this host
APP_HOSTNAME=kin.example.com            # drives BETTER_AUTH_URL, WS, previews, checkout, Traefik Host
...
# --- image ---
APP_TAG=latest                     # (APP_IMAGE defaults to ghcr.io/deeptoai-com/kin/app)
```
...
```bash
curl -sS -o /dev/null -w "%{http_code}\n" https://kin.example.com/health      # 200
curl -sS -o /dev/null -w "%{http_code}\n" https://kin.example.com/            # 200
curl -sS -o /dev/null -w "%{http_code}\n" https://kin.example.com/ws/agent    # 426 (WS upgrade)
```
...
| migrate `28P01 password authentication failed for user "kin"` | A standalone `DATABASE_URL` drifted from `POSTGRES_PASSWORD` | Build `DATABASE_URL` from `POSTGRES_*` in the compose (done) |
...
shell). The published app image is now **multi-arch (amd64 + arm64)**, so it runs natively on
either; no emulation needed.

---

## Known follow-ups

- **CI builds** now publish **prebuilt multi-arch GHCR images** on push to `main`; the Mastra
  SDK and the Playwright/LibreOffice tooling were **removed** to slim the bundle. Contributors
  and deployers pull these images rather than building locally.
- **Phase C routed-preview E2E** on the live host (create a multi-file app → Run preview
  → reach `<id>.kin.example.com`) — pending.
