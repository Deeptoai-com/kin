...
| #   | 子项                                                | 状态                                          |
| --- | --------------------------------------------------- | --------------------------------------------- |
| 1   | per-run token/cost/turns 落库（观测）               | ✅ 已完成（P2-1，PR #55）                      |
| 2   | 计量接线 + 配额（spendOneCredit 从未被调用）        | ✅ 机制已就绪（P2-3，PR #57）；默认关，待负责人定率后开启 |
| 3   | 审计日志表（安全相关动作）                          | ✅ 已完成（P2-2，PR #56）；跨租户拒绝点（worker 内）留待后续 |
