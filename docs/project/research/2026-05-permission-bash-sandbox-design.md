...
## 10. 方向③（独立沙盒容器）—— 计划存档，暂时搁置

> 决定日期：2026-06-01 · 状态：**SHELVED（已存档搁置）**

### 搁置原因

权限模块主体（PR-B 三档选择器 + PR-C srt 沙盒 bash）已完成并验证。
当前阶段用户规模和安全需求，srt + prlimit 的隔离已足够。

独立容器方案工程量大（新服务 + 镜像 + 通信协议 + 文件同步 + 池化），
投入产出比在当前阶段不合算，列入技术债储备，待条件满足再启动。

### 触发条件（满足任意一条时重新评估）

1. **用户完全不可信**（公开注册、无审核）且要跑任意 bash —— 当前爆炸半径在 app 容器内，此时需缩到独立沙盒容器外
2. **高并发**需要池化 / 跨机 / 弹性扩展（目标 > 50 并发 bash 会话）
3. **合规 / 审计**要求更强的物理隔离边界

### 实施时参考

- 设计对标：§9.4 deer-flow AioSandboxProvider + hermes BaseEnvironment
- 架构接口：`ExecutionRuntime`（`src/claude/execution/`）已留好换后端的口子，届时新增一个 backend 不动 agent loop
- deer-flow 代码：`references/useful_frameworks/deer-flow/backend/.../sandbox/`
- hermes 代码：`references/useful_frameworks/hermes-agent/tools/environments/`

### 当前技术状态（归档时）

- srt（bubblewrap）：生产已开（seccomp=unconfined + ENABLE_EXEC_SANDBOX=1）
- 资源限制：prlimit 2C/2G/512 进程 + du 盘配额守卫 + 300s 超时
- 已验证：断网 / 越目录 / fork 炸弹全部拦截（Docker escape test，见 PR #67）
- 待修：R4 wantsBash tier 门控缺口（Issue #69，低风险，计划中）
